...
class Engine():
    ...
    def __init__(self, ki, kv, radius, rotorMass, rotorInternalRadius, rotorExternalRadius):
        self.ki = ki
        self.kv = kv
        self.radius = radius
    ...
    def compute_equilibrium_omega(self, propeller, Vs):
        ...
        roots = np.roots(
            [-propeller.kw, -self.ki / (self.radius * self.kv), self.ki * Vs / self.radius])
    ...
    def getTorque(self, omega, Vs):
        ...
        return (self.ki * Vs / self.radius) - (self.ki / (self.radius * self.kv)) * omega
